...
*/
#define ARCH_MM_PTE_ADDR_MASK 0x000FFFFFFFFFF000ULL
...
void __mm_virt_page_map_assume_aligned(arch_mm_phys_addr_t table,
	...
	KERNEL_ASSERT(ARCH_MM_PAGE_SIZE == 4096);
	...
	if ((pml4[pml4_idx] & AMD64_FLAG_PRESENT) == 0) {
		arch_mm_phys_addr_t pdpt_phys = mm_phys_page_alloc_many(1);
		__builtin_memset(__phys_to_virt(pdpt_phys), 0, ARCH_MM_PAGE_SIZE);
		pml4[pml4_idx] = pdpt_phys | intermediate_flags;
	}

	// Step 4: resolve PDPT (page directory pointer table)
	uint64_t *pdpt = (uint64_t *)__phys_to_virt(pml4[pml4_idx] & ARCH_MM_PTE_ADDR_MASK);

	if ((pdpt[pdpt_idx] & AMD64_FLAG_PRESENT) == 0) {
		arch_mm_phys_addr_t pd_phys = mm_phys_page_alloc_many(1);
		__builtin_memset(__phys_to_virt(pd_phys), 0, ARCH_MM_PAGE_SIZE);
		pdpt[pdpt_idx] = pd_phys | intermediate_flags;
	}

	// Step 5: resolve PD (page directory)
	uint64_t *pd = (uint64_t *)__phys_to_virt(pdpt[pdpt_idx] & ARCH_MM_PTE_ADDR_MASK);

	if ((pd[pd_idx] & AMD64_FLAG_PRESENT) == 0) {
		arch_mm_phys_addr_t pt_phys = mm_phys_page_alloc_many(1);
		__builtin_memset(__phys_to_virt(pt_phys), 0, ARCH_MM_PAGE_SIZE);
		pd[pd_idx] = pt_phys | intermediate_flags;
	}

	// Step 6: resolve PT (page table)
	uint64_t *pt = (uint64_t *)__phys_to_virt(pd[pd_idx] & ARCH_MM_PTE_ADDR_MASK);

	// Step 7: create PTE
	uint64_t pte = (paddr & ARCH_MM_PTE_ADDR_MASK);

	if ((flags & ARCH_MM_FLAG_PRESENT) != 0) {
		pte |= (1ULL << AMD64_SHIFT_PRESENT);
	}
	if ((flags & ARCH_MM_FLAG_WRITE) != 0) {
		pte |= (1ULL << AMD64_SHIFT_WRITE);
	}
	if ((flags & ARCH_MM_FLAG_USER) != 0) {
		pte |= (1ULL << AMD64_SHIFT_USER);
	}
	if ((flags & ARCH_MM_FLAG_EXEC) == 0) { // opposite logic
